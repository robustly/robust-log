...
module.exports = function(moduleName, config, opts) {
  opts = opts || {}
  var logger

  if (moduleName && moduleName.app) {
    config = moduleName
    moduleName = config.app
  }
  ...
  function resolveEnvironmentSettings(env) {
    var filters = process.env['LOG_FILTERS'] ? process.env['LOG_FILTERS'].split(',') : []
    return {
      filters: _.reduce(filters, (s,n) => {s[n] = 1;return s;}, {}),
      level: resolveLogLevel(),
      name: env.NODE_APP || ('lib:'+moduleName),
  ...
  function initLogger() {
    ...
    if (moduleName == m.__settings.name || !m.__settings.appIsDefined || m.__settings.filters[moduleName]) {
  ...
  m.method = m.goal = function(goalLabelStr, data, opts) {
    var goal = createGoal(goalLabelStr, data, opts)
    m(goal.name+'_STARTED', goal)

    return module.exports(moduleName,config, {goal:goal})
  }
  ...
  m.fail = function(err) {
    if (m.__goal) {
      ...
      m(err)
      m('FAILED '+m.__goal.name, m.__goal)
      //m.__goal = null
      delete m
    }
    throw err
  ...
  m.result = m.succeed = m.pass = function(val) {
    if (m.__goal) {
      m.__goal.duration = Date.now() - m.__goal.creationTime
      m.__goal.result = val
      m.log('COMPLETED '+m.__goal.name, m.__goal)
      //m.__goal = null
      delete m
    }
  ...
  function post(level,eventLabel,details,opts) {
    ...
    if (__levels[level] > WARN && config.ringBufferSize) {
      ...
      _.each(__ringbuffer.records, r=> {delete r.level; _log.info(r, r.msg)})
      console.log('\n\n== END RINGBUFFER DUMP ==\n\n')
...
function getCaller3Info() {
  ...
  m.report = function(err, details, origErr) {
    err = err || new Error('ErrorReport')
    if (origErr)
      m.log('An error was thrown:', {err:origErr})
    if (details)
      m.log('Details:', details)
